...
function api.getNearbyInteractions()
    ...
    for _, interaction in pairs(ENTITY_BONES) do
        local interactionCoords = utils.getCoordsFromInteract(interaction)
        local distance = #(interactionCoords - playercoords)
        
        if distance <= interaction.distance then
            -- Check if wall detection is enabled and if there's a clear line of sight
            local isVisible = true
            if settings.wallDetection and settings.wallDetection.enabled then
                local rayHandle = StartShapeTestRay(
                    playercoords.x, playercoords.y, playercoords.z,
                    interactionCoords.x, interactionCoords.y, interactionCoords.z,
                    settings.wallDetection.rayFlags, cache.ped, 0
                )
                local _, hit, _, _, entityHit = GetShapeTestResult(rayHandle)
                -- Check if the hit entity is a vehicle or the interaction entity or the player's vehicle
                local isVehicle = entityHit and GetEntityType(entityHit) == 2
                isVisible = not hit or (interaction.entity and entityHit == interaction.entity) or entityHit == playerVehicle or isVehicle
            end
        ...
        if MODELS[hash] then
            ...
            if distance <= interaction.distance then
                ...
                if settings.wallDetection and settings.wallDetection.enabled then
                    local rayHandle = StartShapeTestRay(
                        playercoords.x, playercoords.y, playercoords.z,
                        nearby.coords.x, nearby.coords.y, nearby.coords.z,
                        settings.wallDetection.rayFlags, cache.ped, 0
                    )
                    local _, hit, _, _, entityHit = GetShapeTestResult(rayHandle)
                    isVisible = not hit or entityHit == nearby.object
                end
        ...
        for i = 1, amountOfInteractions do
            ...
            if distance <= interaction.distance then
                ...
                if settings.wallDetection and settings.wallDetection.enabled then
                    ...
                    local _, hit, _, _, entityHit = GetShapeTestResult(rayHandle)
                    isVisible = not hit or (interaction.entity and entityHit == interaction.entity)
                end
